//
//  VisionBarcodeCameraViewManager.swift
//  TOTVSClockInVision
//
...
@objc(VisionBarcodeCameraViewManager)
class VisionBarcodeCameraViewManager : RCTViewManager {
        
    override func view() -> UIView! {
        return VisionBarcodeCameraView()
    }
...
/// MARK: withView
extension VisionBarcodeCameraViewManager {
    func withView(_ node: NSNumber, action: @escaping (VisionBarcodeCameraView) -> Void) {
        DispatchQueue.main.async {
            let view = self.bridge.uiManager.view(
                forReactTag: node
            ) as! VisionBarcodeCameraView
...
fileprivate let BARCODE_FORMAT = [
    "QR_CODE": 1,
    "AZTEC": 2
]
